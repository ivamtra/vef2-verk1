...
export function buildPage(csvFile) {
  ...
  const fileName = `${DISTPATH}${csvFile.substring(lastSlash+1, csvFile.length-4)  }.html`
 ...
 export function buildHomePage(jsonFile) {
  ...
  const fileName = `${DISTPATH}${jsonFile.substring(lastSlash+1, jsonFile.length-5)  }.html`
